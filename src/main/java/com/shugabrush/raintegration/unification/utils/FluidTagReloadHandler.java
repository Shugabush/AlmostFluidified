...
public final class FluidTagReloadHandler
{
    ...
    public static void run()
    {
        ...
        RAIntegration.onTagLoaderReload(RAW_FLUID_TAGS);
    ...
    public static void applyCustomTags(FluidUnifyConfig unifyConfig)
    {
        ...
        for (var entry : unifyConfig.getCustomTags().entrySet())
        {
            ...
            for (ResourceLocation fluidId : fluidIds)
            {
                ...
                {
                    RAIntegration.LOGGER.warn("[CustomTags] Custom tag '{}' contains invalid fluid '{}'", tag, fluidId);
                    continue;
                }
                ...
                if (currentHolders != null)
                {
                    ...
                    {
                        RAIntegration.LOGGER.warn("[CustomTags] Custom tag '{}' already contains fluid '{}'", tag,
                                fluidId);
                        continue;
                    }
        ...
        if (!changedFluidTags.isEmpty())
        {
            ...
            {
                RAIntegration.LOGGER.info("[CustomTags] Modified tag '#{}', added {}", tag, fluids);
            });
        }
    }
}
