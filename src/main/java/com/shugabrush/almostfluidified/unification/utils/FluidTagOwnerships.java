...
public class FluidTagOwnerships
{
    ...
    public void applyOwnerships(Map< ResourceLocation, Collection< Holder< Fluid>>> rawTags)
    {
        ...
        ownerToRefs.asMap().forEach((owner, refs) ->
        {
            ...
            if (rawHolders == null)
            {
                AlmostFluidified.LOGGER.warn("[FluidTagOwnerships] Owner tag '#{}' does not exist!", owner.location());
                return;
            }
        ...
        if (!changedTags.isEmpty())
        {
            ...
            {
                AlmostFluidified.LOGGER.info("[FluidTagOwnerships] Modified tag '#{}', added {}", tag, fluids);
            });
        }
    }
